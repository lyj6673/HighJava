...
/*
 	...
 	2. 컨테이너는 web.xml에 정의된 'URL패턴'을 확인하여 어느 서블릿을 통해서 처리해야 할지를
 		...
 		(서블릿 3.이상에서는 애노테이션(@WebServlet)으로 설정이 가능하다.)
 	...
 	5. 요청 및 응답 처리가 완료되면 HttpServletRequest객체와 HttpServletRespons객체는
 		자동으로 소멸된다.
 	6. Servlet Container에서 서블릿이 제거되는 경우에는 destory()메서드가 호출된다.
 */
...
public class ServletTest03 extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 방법2) 클라이언트의 전송 방식(GET, POST등)을 구분해서 직접 메서드 호출하기
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		out.println("<body style='color:red;'><h1>doGet()메서드를 처리한 결과입니다.<h1></body></html>");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		PrintWriter out = response.getWriter();
		out.println("<html><head><meta charset='utf-8'></head>");
		out.println("<body style='color:blue;'><h1>doPost()메서드를 처리한 결과입니다.<h1></body></html>");
	}
